...
const BookUser = () => {
  ...
  const toggleDish = (dishId) => {
    setFormData((prev) => {
      const isSelected = prev.selectedDishes.some(dish => dish.dishId === dishId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('❌ Lỗi khi đặt bàn:', err.response ? err.response.data : err.message); // Log chi tiết lỗi hơn
      alert('❌ Lỗi khi gửi dữ liệu. Vui lòng kiểm tra lại thông tin hoặc thử lại sau.'); // Thông báo lỗi rõ hơn
    }
  };
  ...
  const categories = ['Tất cả', 'Bữa Sáng', 'Bữa Trưa', 'Đồ Uống', 'Tráng Miệng']; // Cân nhắc lấy categories động từ menuItems
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filteredMenu.length > 0 ? (
             filteredMenu.map((dish) => {
                const selectedDish = formData.selectedDishes.find((item) => item.dishId === dish._id);
                ...
                return (
                  ...
                  >
                    <div> {/* Div bao nội dung món ăn */}
                        <img
                        src={dish.image || 'https://via.placeholder.com/150?text=No+Image'} // Ảnh mặc định
                        alt={dish.name}
                        className="w-full h-32 object-cover rounded mb-2"
                        />
                        <h4 className="text-sm font-medium truncate">{dish.name}</h4> {/* truncate nếu tên dài */}
                        <p className="text-xs text-gray-500 mb-1">{dish.category}</p>
                        <p className="text-sm font-bold text-red-600"> {/* Màu đỏ đậm hơn */}
                    ...
                    <div className={`flex items-center justify-center mt-2 space-x-3 ${isSelected ? 'opacity-100' : 'opacity-0 h-0' } transition-opacity duration-300`}>
            ...
            <p className="col-span-full text-center text-gray-500">Không tìm thấy món ăn phù hợp.</p>
          )}
        </div>
      </div>

      {/* Nút submit */}
      <div className="text-right mt-6"> {/* Thêm margin top */}
        <button
          type="submit"
